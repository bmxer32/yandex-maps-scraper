...
# Бизнесы, куда клиент записывается на время. Для магазина, склада или
# доставки отсутствие записи ничего не значит — их этим правилом не трогаем.
_BOOKING_BUSINESS = re.compile(
    r"(салон|парикмахер|барбер|ногт|маникюр|педикюр|космет|бров|ресниц|эпиляц|депиляц"
    r"|массаж|спа|тату|пирсинг|стомат|клиник|медцентр|медицинск|поликлиник|врач|приём"
    r"|фитнес|йог|пилатес|тренаж|бассейн|студи|груминг|ветеринар|автосервис|шиномонтаж"
    r"|детейлинг|учебн|курсы|репетитор|нотариус|юрист|фотограф)",
    re.IGNORECASE,
)


def needs_booking(org: Organization) -> bool:
    """Записываются ли в такой бизнес на конкретное время."""
    blob = " ".join([org.name or ""] + list(org.categories or []))
    return bool(_BOOKING_BUSINESS.search(blob))
