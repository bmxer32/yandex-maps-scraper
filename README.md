...
- 🧠 **Отбор клиентов** — вердикт по каждой организации: живой ли бизнес, не сеть ли, годится ли под демо
...
2. **Проба главной** — отвечает ли сайт, не закрыт ли антиботом, свежий ли копирайт, нет ли упоминания филиалов. Один запрос на компанию.
3. **Вердикт модели** — сеть или одиночка, живой или заброшен, есть ли что продавать. Только для тех, у кого страница скачалась.

Появляется колонка «Вердикт» с причиной в подсказке, фильтры и кнопка **«Выбрать годных»**.
...
```bash
python checks/prospects_check.py   # отбор клиентов: 44 проверки
