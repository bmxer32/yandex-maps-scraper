...
Кнопка **«ТГ без сайта»** сохраняет в `.txt` телеграм-контакты тех, у кого **нет своего сайта** — по одному в строке, для загрузки в рассыльщик. «Без сайта» здесь то же, что в отборе: пустое поле, ВКонтакте, виджет записи или конструктор. На выгрузке салонов своего сайта нет у 16 компаний из 35, а совсем пустое поле — лишь у двух, так что считать по одному пустому полю значит потерять почти весь сегмент. В файл идут и юзернеймы (`@salon`), и телефоны (`+79991234567`): у половины таких компаний в телеграме указан именно номер, а рассыльщики принимают оба вида. Приглашения в закрытые чаты пропускаются. Считаются **видимые** строки — фильтры и оценка учитываются.
