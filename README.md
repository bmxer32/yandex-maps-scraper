...
[![Version](https://img.shields.io/badge/version-0.3.2-orange.svg)](https://github.com/bmxer32/yandex-maps-scraper/releases)
...
- 💾 **Экспорт** — Excel (.xlsx), CSV и `.txt` с телеграм-контактами тех, у кого нет своего сайта — под рассылку
...
- ⭐ **В работе** — конторы, за которые взялись: статус, заметка, напоминание. Переживает вытеснение выгрузки из истории

## 🚀 Установка

### Готовый установщик (для пользователей)

1. Скачайте `YandexMapsScraper-Setup-0.3.2.exe` из [Releases](https://github.com/bmxer32/yandex-maps-scraper/releases)
...
Интерфейс разбит на четыре вкладки: **Поиск**, **История**, **В работе** и **Демо**.
...
Кнопка **«ТГ без сайта»** сохраняет в `.txt` телеграм-контакты тех, у кого **нет своего сайта** — по одному в строке, для загрузки в рассыльщик. «Без сайта» значит, что писать туда осмысленно: поля нет вовсе, либо в нём ВКонтакте, виджет записи, конструктор, «ссылка в шапке профиля» или мессенджер — `viber.click/79376603373`, `vk.link/clubzebrasun`, `n1054718.yclients.com`, `dikidi.ru/976004`. Список свой и на отбор клиентов не влияет: там свои оси и свой кэш вердиктов. Считается одинаково и до оценки, и после неё. На выгрузке салонов своего сайта нет у 16 компаний из 35, а совсем пустое поле — лишь у двух, так что считать по одному пустому полю значит потерять почти весь сегмент. В файл идут и юзернеймы (`@salon`), и телефоны (`+79991234567`): у половины таких компаний в телеграме указан именно номер, а рассыльщики принимают оба вида. Приглашения в закрытые чаты пропускаются. Считаются **видимые** строки — фильтры и оценка учитываются.
...
### Вести конторы в работе

Звёздочка слева от названия берёт контору **в работу** — она попадает во вкладку «В работе» и живёт там своей жизнью.
...
|---|---|
| **Новый** | взяли, но ещё не трогали |
| **Написал** | отправили сообщение или демо |
| **Ответили** | клиент отозвался, разговор идёт |
| **Клиент** | договорились |
| **Отказ** | не сложилось |

Сверху фильтр по статусу, справа — счётчик просроченных: у кого дата напоминания уже прошла. Такие строки поднимаются наверх и подсвечиваются.

Повторный парсинг той же ниши **обновляет** телефоны, сайты и оценку у контор в работе, но **не трогает** статус, заметку и напоминание. Пересобрал нишу — не потерял, кому уже писал.
